...
## emea-entry-level.md / na-entry-level.md (broader view)

Two markdown files at the repo root, generated by the same run, sit
beside JOBS.md as **wider-net browses** of intern + new-grad roles —
one per region. Both are produced by the shared
`render_region_entry_level(rows, region, path)` (back-compat
`render_emea_entry_level` and `render_na_entry_level` aliases exist for
callers).
...
- `render_emea_entry_level()` / `render_na_entry_level()` filter this
  pool to one region and write the files at end of run, after
  `render_md()` produces JOBS.md.
...
- Paths are overridable via `--md-emea-entry-level` /
  `--md-na-entry-level`; defaults are `<repo_root>/emea-entry-level.md`
  and `<repo_root>/na-entry-level.md` to mirror JOBS.md's location.
...
`emea-entry-level.md` / `na-entry-level.md` live there too, for the
same reason. See the dedicated section above for what's different
about them.
